...
RUN_ID=${RUN_ID:-stagewise_coldaq_colabmp_n128_s4096_t20_no_full_visible}
...
DAMPING_MODE=${DAMPING_MODE:-absolute}
...
GPTQ_COMPENSATION=${GPTQ_COMPENSATION:-0}
GPTQ_COMPENSATION_DEVICE=${GPTQ_COMPENSATION_DEVICE:-cuda:0}
DAQ_FULLS_REFINE=${DAQ_FULLS_REFINE:-0}
DAQ_FULLS_DEVICE=${DAQ_FULLS_DEVICE:-cuda:0}
DAQ_FULLS_STEPS=${DAQ_FULLS_STEPS:-3}
DAQ_FULLS_RIDGE=${DAQ_FULLS_RIDGE:-1e-5}
DAQ_FULLS_TARGET_MODULES=${DAQ_FULLS_TARGET_MODULES:-ff_proj,up_proj,ff_out}
DAQ_FULLS_START_BLOCK=${DAQ_FULLS_START_BLOCK:-24}
DAQ_FULLS_END_BLOCK=${DAQ_FULLS_END_BLOCK:-32}
...
EVAL_OUTPUT_DIR=${EVAL_OUTPUT_DIR:-/root/data-fs/Quant-dllm/benchmark_results/${RUN_ID}_${EVAL_LIMIT_TAG}}
...

{
  ...
  echo "DAQ_FULLS_REFINE=$DAQ_FULLS_REFINE DAQ_FULLS_DEVICE=$DAQ_FULLS_DEVICE DAQ_FULLS_STEPS=$DAQ_FULLS_STEPS DAQ_FULLS_RIDGE=$DAQ_FULLS_RIDGE"
  echo "DAQ_FULLS_TARGET_MODULES=$DAQ_FULLS_TARGET_MODULES DAQ_FULLS_BLOCK_RANGE=[$DAQ_FULLS_START_BLOCK,$DAQ_FULLS_END_BLOCK)"
  echo "Started: $(date)"

  quant_cmd=(
    "$PYTHON_BIN" -u /root/data-fs/Quant-dllm/tools/quant_stagewise_pipeline.py
  ...
  if [ "$DAQ_FULLS_REFINE" = "1" ]; then
    quant_cmd+=(
      --daq_fulls_refine
      --daq_fulls_device "$DAQ_FULLS_DEVICE"
      --daq_fulls_steps "$DAQ_FULLS_STEPS"
      --daq_fulls_ridge "$DAQ_FULLS_RIDGE"
      --daq_fulls_target_modules "$DAQ_FULLS_TARGET_MODULES"
      --daq_fulls_start_block "$DAQ_FULLS_START_BLOCK"
      --daq_fulls_end_block "$DAQ_FULLS_END_BLOCK"
    )
  fi
